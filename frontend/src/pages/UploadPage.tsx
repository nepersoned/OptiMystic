    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-8 gap-6 max-w-2xl mx-auto w-full">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-slate-900">{t('uploadTitle')}</h1>
          <p className="text-slate-500 mt-1 text-sm">{t('uploadSubtitle')}</p>
        </div>
        ...
        {!result && (
          ...
          >
            ...
            <div className="text-center">
              <p className="font-medium text-slate-700">{t('dropzone')}</p>
          ...
          <Card className="w-full">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: t('rows'), value: result.row_count },
                  { label: t('cols'), value: result.col_count },
                  { label: t('version'), value: `v${result.version}` },
                ].map((s) => (
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-slate-500">{t('inferredDomain')}</span>
              ...
              <div>
                <p className="text-xs text-slate-500 mb-2">{t('detectedColumns')}</p>
                <div className="flex flex-wrap gap-1">
                  {result.columns.map((c) => (
                    <span key={c} className="bg-slate-100 text-slate-700 text-xs px-2 py-0.5 rounded">{c}</span>
                  ))}
                </div>
              </div>
