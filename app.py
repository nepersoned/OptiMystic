...
# 1. Generic Wizard (For Custom Mode & Others)
wizard_input_section = html.Div([
    ...
    dcc.RadioItems(id='input-role', options=[{'label': ' Decision Variable', 'value': 'var'}, {'label': ' Parameter', 'value': 'param'}], value='var', labelStyle={'display': 'inline-block', 'marginRight': '20px', 'marginBottom': '15px'}),
    html.Label("Step 2. What is the Shape?", style={'fontWeight': 'bold', 'color': '#28a745'}),
    dcc.RadioItems(id='input-shape', options=[{'label': ' Scalar', 'value': 'scalar'}, {'label': ' Matrix', 'value': 'matrix'}, {'label': ' List', 'value': 'list'}], value='scalar', labelStyle={'display': 'inline-block', 'marginRight': '20px', 'marginBottom': '15px'}),
    html.Hr(style={'margin': '20px 0', 'borderTop': '1px dashed #ccc'}),
    html.Label("Name:", style={'fontWeight': 'bold'}),
    dcc.Input(id='input-name', type='text', placeholder="Name...", style=input_style),
    # Matrix UI
    html.Div(id='matrix-config-area', children=[
        html.Div([html.Div([html.Label("Rows:"), dcc.Input(id='matrix-rows', type='text', style=input_style)], style={'width':'48%'}), html.Div([html.Label("Cols:"), dcc.Input(id='matrix-cols', type='text', style=input_style)], style={'width':'48%'})], style={'display':'flex', 'gap':'4%'}),
        html.Button("Generate Grid", id='btn-gen-matrix', n_clicks=0, style={'marginTop': '5px'}),
    ...
    html.Div(id='list-config-area', children=[
        html.Label("Columns:"), dcc.Input(id='list-cols', type='text', style=input_style),
        html.Button("Init List", id='btn-init-list', n_clicks=0, style={'marginTop': '5px'}),
        html.Div(id='list-input-container', style={'marginTop':'15px', 'display':'none'}, children=[dash_table.DataTable(id='list-data-table', columns=[{'name':'Col1','id':'col1'}], data=[], editable=True, row_deletable=True), html.Button("+ Row", id='btn-add-list-row', n_clicks=0, style={'marginTop':'10px'})])
    ], style={'display':'none'}),
    # Value UI
    ...
    # Type UI
    html.Div(id='type-input-box', style={'display':'block'}, children=[html.Label("Type:"), dcc.Dropdown(id='input-var-type', options=[{'label':'Continuous','value':'Continuous'},{'label':'Integer','value':'Integer'}], value='Continuous')]),
    # Add Button
    html.Button('Add to Table', id='add-btn', n_clicks=0, style={'width': '100%', 'marginTop':'20px', 'padding': '12px', 'backgroundColor': '#4a4e69', 'color': 'white', 'border': 'none', 'borderRadius': '8px', 'cursor': 'pointer'}),
    html.Div(id='add-msg', style={'marginTop': '10px'})
], style=question_style)

# 2. Production Mix Custom UI (New!)
prod_mix_input_section = html.Div([
    html.H4("🏭 Production Mix Setup", style={'marginBottom': '20px', 'color': '#4a4e69'}),
    html.P("Define your products and resource usage below. No formulas needed!", style={'color': '#666', 'fontSize': '14px'}),
    
    html.Div([
        html.H5("1. Define Products", style={'color': '#007bff'}),
        html.P("Add products with their selling price (Profit).", style={'fontSize': '13px'}),
        ...
        html.Button("+ Add Product", id='pm-add-prod-btn', n_clicks=0, style={'marginTop': '10px', 'padding': '5px 10px', 'cursor': 'pointer'})
    ], style={'marginBottom': '30px', 'padding': '15px', 'border': '1px dashed #007bff', 'borderRadius': '8px'}),

    html.Div([
        html.H5("2. Resource Constraints", style={'color': '#28a745'}),
        html.P("How much resource each product uses? (Leave 0 if not used)", style={'fontSize': '13px'}),
        # This table will be dynamic based on products
        html.Div(id='pm-resource-matrix-container')
    ], style={'padding': '15px', 'border': '1px dashed #28a745', 'borderRadius': '8px'})
...
modeling_section = html.Div([
    ...
    html.Div([
        html.Label("1. Objective:", style={'fontWeight': 'bold', 'color': '#007bff'}),
        dcc.RadioItems(id='solver-sense', options=[{'label': ' Minimize', 'value': 'minimize'}, {'label': ' Maximize', 'value': 'maximize'}], value='minimize'),
        dcc.Textarea(id='solver-objective', placeholder="Objective Function...", style={'width': '100%', 'height': '60px', 'marginTop': '10px'}),
        html.Label("2. Constraints:", style={'fontWeight': 'bold', 'color': '#d63384', 'marginTop': '20px'}),
        dcc.Textarea(id='solver-constraints', placeholder="Constraints...", style={'width': '100%', 'height': '150px'}),
        html.Button("🚀 Run Optimization", id='btn-solve', n_clicks=0, style={'width': '100%', 'marginTop':'20px', 'padding': '15px', 'backgroundColor': '#28a745', 'color': 'white', 'border': 'none', 'borderRadius': '8px', 'fontSize': '18px', 'cursor': 'pointer'})
    ], style=question_style)
])

# --- Dashboard Section (Step 3) ---
dashboard_section = html.Div([
    html.H4("📊 Optimization Dashboard", style={'marginBottom': '20px', 'color': '#4a4e69'}),
    html.Div([
        html.Div([html.H6("Status"), html.H2(id='res-status', children="-")], style=card_style),
        html.Div([html.H6("Objective"), html.H2(id='res-objective', children="-")], style=card_style),
    ], style={'display': 'flex', 'gap': '20px', 'marginBottom': '30px'}),
    html.Div(id='result-dashboard', style={'display': 'none'}, children=[
        html.H5("Decision Variables"),
        dash_table.DataTable(id='res-table', columns=[{'name': 'Var', 'id': 'Variable'}, {'name': 'Val', 'id': 'Value'}], data=[], style_header=table_header_style, style_cell=table_cell_style),
        html.H5("Sensitivity", style={'marginTop': '20px'}),
        dash_table.DataTable(id='res-constraints-table', columns=[{'name': 'ID', 'id': 'Constraint'}, {'name': 'Shadow Price', 'id': 'Shadow Price'}, {'name': 'Slack', 'id': 'Slack (Remaining)'}], data=[], style_header=table_header_style, style_cell=table_cell_style)
    ]),
    html.Div(id='solver-error-msg', style={'color': '#dc3545', 'marginTop': '20px', 'whiteSpace': 'pre-wrap'})
], style={'padding': '30px', 'backgroundColor': '#f8f9fa', 'borderRadius': '12px'})

# --- Helper Functions ---
def render_landing_page():
    cards = [html.Div([html.Div(t['icon'], style={'fontSize': '40px'}), html.H3(t['title'], style={'fontSize':'18px'}), html.P(t['desc'], style={'fontSize':'14px', 'color':'#666'}), html.Button("Select", id={'type': 'tmpl-btn', 'index': t['id']}, style={'marginTop':'15px', 'padding':'8px 16px', 'backgroundColor':'#4a4e69', 'color':'white', 'border':'none', 'borderRadius':'6px'})], style=card_style) for t in TEMPLATE_GALLERY]
    return html.Div([html.H1("Select Optimization Problem", style={'textAlign': 'center'}), html.Div(cards, style={'display': 'grid', 'gridTemplateColumns': 'repeat(auto-fit, minmax(280px, 1fr))', 'gap': '20px'})])
...
def render_workspace(mode):
    ...
    # [Switching Content] Check Mode
    if mode == 'prod_mix':
        input_content = prod_mix_input_section  # Custom UI for Phase 6
    else:
        input_content = html.Div([  # Default Wizard for others
            wizard_input_section,
            html.H4("Defined Data", style={'marginTop': '30px'}),
    ...
    return html.Div([
        ...
        html.Div([html.Span(f"Selected Mode: {title}", style={'backgroundColor': '#e2e6ea', 'padding': '5px 10px', 'borderRadius': '15px'})], style={'marginBottom': '20px'}),
        dcc.Tabs([
...
app.layout = html.Div([
    ...
    html.Div([
        html.H2("🧙‍♂️ OptiMystic", style={'margin': 0, 'color': '#4a4e69'}),
        html.Button("🏠 Home", id='btn-home', style={'padding': '5px 10px', 'cursor': 'pointer', 'border': 'none', 'fontSize': '16px'})
...
def change_url(tmpl_clicks, home_click, current_path):
    ...
    if btn_id == 'btn-home':
        return "/home"  # Explicitly using /home
...
# --- Wizard Logic (Only active for Custom Mode) ---
...
def toggle_inputs(role, shape):
    return ({'display':'block','padding':'15px','backgroundColor':'#e9ecef','borderRadius':'8px','marginBottom':'15px'} if shape=='matrix' else {'display':'none'}), \
           ({'display':'block','padding':'15px','backgroundColor':'#e3f2fd','borderRadius':'8px','marginBottom':'15px'} if shape=='list' else {'display':'none'}), \
...
def run_solver(n, sense, obj, const, store):
    ...
    if res.get('status') == 'Error': return {'display':'none'}, "Error", {'color':'red'}, "-", [], [], f"System Error:\n{res.get('error_msg')}"
    return {'display':'block'}, res['status'], ({'color':'green'} if res['status']=='Optimal' else {'color':'orange'}), f"{res['objective']:,.2f}", res['variables'], res['constraints'], ""
